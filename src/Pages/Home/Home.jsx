...
import { CategName, Category, CategoryWrapper, ImgWrapper } from "./Home.styled";
import { fetchAdventureAnime, fetchRomanceAnime, fetchHorrorAnime, fetchAdventureManga, fetchActionManga } from "../../api/request";
...
function Home() {
    ...
    useEffect(() => {
        const fetchAllData = async () => {
            try {
                const [adventureData, romanceData, horrorData, adventureMangaData, actionMangaData] = await Promise.all([
                    fetchAdventureAnime(),
                    fetchRomanceAnime(),
                    fetchHorrorAnime(),
                    fetchAdventureManga(),
    ...
    const renderCategory = (title, data) => (
        <>
            ...
            <Category>
                {data.map((item) => (
                    <Link key={item.id} to="/preview" onClick={() => handleAnimeClick(item)}>
                        <ImgWrapper>
                            <img
    ...
    return (
        <div style={{ overflowX: "hidden" }}>
            ...
            <CategoryWrapper>
                {loading ? (
                    <ClipLoader color="purple" size={50} />  // Use the spinner here
                ) : (
                    <>
